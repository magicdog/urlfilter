...
import com.hipu.bdb.util.FPGenerator;
import com.hipu.bdb.util.FileUtils;
...
public class BdbUriUniqFilter {
    ...
    public void start() {
        ...
        boolean isRecovery = false;  //是否从checkpoint恢复
        try {
...
//            }
            ...
            logger.info("the amount of pairs in db "+db.count());
            initialize(db);
        } catch (DatabaseException e) {
    ...
    public boolean setAdd(CharSequence uri) {
        ...
        try {
            ...
            //status = alreadySeen.put(null, key, ZERO_LENGTH_ENTRY);
            if (logger.isInfoEnabled()) {
    ...
    public boolean contains( final CharSequence uri ) {
    	DatabaseEntry key = new DatabaseEntry();
        LongBinding.longToEntry(createKey(uri), key);
        OperationStatus status = null;
        status = alreadySeen.get(null, key, ZERO_LENGTH_ENTRY, null);
        if (status == OperationStatus.SUCCESS) {
        	return true;
        } else
        	return false;
    ...
    public void truncate() {
    	Environment env = this.alreadySeen.getEnvironment();
    	if (env == null)
    		return;
    	else {
    		 try {
