...
package com.hipu.bdb.bloomfilter;
...
/** URL去重方案
 * 基于磁盘：
 * 	1、基于磁盘路径。计算Hash，根据值新建目录和文件。
 *  导致磁盘系统崩溃。
 * 	2、基于数据库。存储到数据库中，进行查询。
 * 	可以利用数据库提供的索引和分布式机制，但是总体效率较低。
 * 基于内存：
 *  1、基于Hash算法。为每个URL计算Hash值，采用HashSet或者Map结构存储。
 *  内存的消耗与URL的数量成正比，随着抓取数量的增加，终究会到达瓶颈。
 *  2、Redis。使用Redis的sorted sets实现。
 *  3、Bloom Filter。空间占用较少。
 *  有一定的false Positive。
 * 两者：
 * 	1、Barkeley DB。直接存成B-Tree结构，和程序运行在同一个进程空间，采用key-value模型。
 *  Berkeley DB Java Edition efficiently caches most commonly used data in memory, without exceeding application-specified limits.。
 * 在heritrix实现的过滤实现：
 * 	1、BdbUriUniqFilter.采用BDB完成去重。
 *  2、BloomUriUniqFilter.采用Bloom Filter完成。
 *  3、FPUriUniqFilter <-- FPMergeUriUniqFilter <-- DiskFPMergeUriUniqFilter 采用disk存储
 *  4、											<-- MemFPMergeUriUniqFilter  采用内存存储
 *  5、MemUriUniqFilter.采用HashSet.
 *  6、NoopUriUniqFilter.只提供空方法。
 * */
...
public class BloomFilter64bit implements Serializable, BloomFilter {
    ...
    public BloomFilter64bit( final long n, final int d, Random weightsGenerator, boolean roundUp ) {
        ...
        this.d = d;
        System.out.println("charsequence size :"+n+"hash count:" +d);
        long lenInLongs = (long)Math.ceil( ( (long)n * (long)d / NATURAL_LOG_OF_2 ) / 64L );
        System.out.println("total length in long: "+ lenInLongs);
        if ( lenInLongs > (1L<<48) ) {
        ...
        System.out.println("total length in bits: "+lenInBits);
        if(roundUp) {
            int pow = 0;
        ...
        System.out.println("need longs "+lenInLongs+" power "+ power);

        
        int arrayOfArraysLength = (int)((lenInLongs+SUBARRAY_LENGTH_IN_LONGS-1)/SUBARRAY_LENGTH_IN_LONGS);
        System.out.println("array count "+arrayOfArraysLength);
    ...
    public static void main(String args[]) {
        BloomFilter64bit bf = new BloomFilter64bit(40, 3, true);
        System.out.println(bf.getHashCount());
        bf.add("http://");
        System.out.println(bf.size());
        
        System.out.println(bf.getSizeBytes());
    }
}
